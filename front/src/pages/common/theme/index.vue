<template>
  <view class="theme-bg min-h-screen p-5" :class="themeClass">
    <view class="mb-5">
      ...
      <view class="theme-bg-secondary overflow-hidden rounded-12rpx">
        ...
        >
          ...
          <view class="flex-1">
            <text class="theme-text mb-2 block text-28rpx font-medium">{{ option.label }}</text>
            <text class="theme-text-tips block text-24rpx">{{ option.description }}</text>
          </view>
          <view v-if="theme === option.value" class="ml-5">
            <nut-icon name="check" color="var(--theme-primary)" size="20" />
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="mb-5">
      ...
      <view class="theme-bg-secondary rounded-12rpx p-7.5">
        ...
        <view class="mb-7.5 flex flex-wrap gap-5">
          ...
          <view class="min-w-120rpx flex-1">
            <nut-button type="error">错误按钮</nut-button>
          </view>
        </view>
    ...
    <view class="py-5 text-center">
      <nut-button :type="'primary'" @click="toggleTheme">
...
const themeOptions = [
  { label: '亮色主题', value: 'light' as ThemeMode, icon: 'i-mdi-white-balance-sunny', description: '适合明亮环境使用' },
  { label: '暗色主题', value: 'dark' as ThemeMode, icon: 'i-mdi-moon-and-stars', description: '适合暗光环境使用' },
];
</script>
